...
export function useWalletTrade() {
  ...
  const copyFromSignal = useCallback(
    async (signal: DeskSignal | null | undefined) => {
      if (!signal) return { ok: false, message: "No signal to copy" };
      if (!isConnected || !address) {
        return { ok: false, message: "Connect your Shannon wallet to copy" };
      }
      ...
      if (!side) return { ok: false, message: "No last trade or recommendation to copy" };
      ...
      try {
        ...
        return {
          ...
          message: result.txHash
            ? `Copy ${side} sent · ${result.txHash.slice(0, 10)}…`
            : `Copy ${side} sent`,
